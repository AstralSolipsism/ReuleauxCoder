...
def build_subagent_approval_provider(
    ...
    """
    parent_provider: ApprovalProvider | None = getattr(
    ...
    parent_handler = getattr(parent_provider, "handler", None)
    if parent_handler is None:
        base_provider = parent_provider

        def parent_handler(pending: PendingApproval) -> None:
            pending.resolve(base_provider.request_approval(pending.request))
